...
import { User, InitialUsernamePipe } from '@alfresco/adf-core';
...
export class PeopleSmartComponent implements OnInit, OnChanges {
    /** Translation key for invalid preselected user error message. */
    readonly preselectedUserErrorKey = input<string>('SHARED_IDENTITY.PEOPLE.ERROR.NOT_FOUND');
    ...
    private getPreselectedUsers(): IdentityUserModel[] {
        return this.isSingleMode() ? [this.preSelectUsers?.[0]].filter(Boolean) : this.removeDuplicatedUsers(this.preSelectUsers ?? []);
    }
        ...
        for (const preselectedUser of this.getPreselectedUsers()) {
            if (preselectedUser) {
                try {
                    ...
                    const validationResult = searchResult.find((user: any) => user && user.username === preselectedUser.username);
    ...
    private generateInvalidUsersMessage(): void {
        ...
        for (const [index, invalidUser] of this.invalidUsers.entries()) {
            this.validateUsersMessage += index === this.invalidUsers.length - 1 ? `${invalidUser.username} ` : `${invalidUser.username}, `;
        }
    }

...
describe('IdentityUserService', () => {
    ...
    describe('Current user info (JWT token)', () => {
        beforeEach(() => {
            ...
            jest.spyOn(localStorage, 'getItem').mockImplementation((key: string): string => store[key] || null);
            jest.spyOn(localStorage, 'setItem').mockImplementation((key: string, value: string): string => (store[key] = value));
        });
    ...
    describe('Search', () => {
        it('should fetch users', (done) => {
            spyOnGet.mockReturnValue(of(mockFoodUsers));

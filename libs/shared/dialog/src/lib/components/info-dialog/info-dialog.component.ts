...
export class InfoDialogComponent implements OnInit {
    ...
    private generateMessageGroups(
        ...
        { modelType = '', translatedModelType, modelName = '', key, params, description }: ErrorLogGroup
    ): Record<string, MessageGroupData[]> {
        const displayType = translatedModelType || modelType;
        const groupKey = `${displayType ? displayType + ' -- ' : ''}${modelName}`;

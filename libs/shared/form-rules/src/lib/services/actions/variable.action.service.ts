...
import { FormFieldModel, FormFieldTypes, FormRulesEvent, FormService, WidgetVisibilityService } from '@alfresco/adf-core';
...
export class VariableActionsService {
    ...
    private widgetVisibilityService = inject(WidgetVisibilityService);

    execute(action: VariableAction, event: FormRulesEvent, data?: FormProcessFinishEventData) {
            ...
            Object.keys(action.payload).forEach((variableActionPayload) => {
                switch (variableActionPayload) {
                    case VariablesInputs.VALUE: {
                        ...
                        this.widgetVisibilityService.refreshVisibility(event.form);
                        break;
                    }
                    default: {
                        break;
    ...
    private updateVariableDependantFields(formFields: FormFieldModel[], variable: any): void {
        const dropdownFields = formFields.filter((field) => {
            return field.type === FormFieldTypes.DROPDOWN && field.optionType === 'variable' && field.variableConfig.variableName === variable.name;
        });

...
describe('Document Reducer', () => {
    ...
    describe('hasMachineTextLayer updates', () => {
        it('should set hasMachineTextLayer when provided', () => {
            ...
            const action = userActions.updatePagesRotation({
                pages: [{ pageId, documentId: doc.id, hasMachineTextLayer: true }],
                taskDataSynced: false,
            });

            const state = documentReducer(mockDocumentState, action);
            const updatedDoc = state.entities[doc.id];
            expect(updatedDoc?.pages.find((p) => p.id === pageId)?.hasMachineTextLayer).toBe(true);
        });
        ...
        it('should retain existing hasMachineTextLayer when not provided', () => {
            ...
            const doc = documents[0];
            const pageId = doc.pages[0].id;
            doc.pages[0].hasMachineTextLayer = true;
            ...
            const action = userActions.updatePagesRotation({
                pages: [{ pageId, documentId: doc.id, rotation: 45 }],
                taskDataSynced: false,
            });
            ...
            const updatedDoc = state.entities[doc.id];
            expect(updatedDoc?.pages.find((p) => p.id === pageId)?.hasMachineTextLayer).toBe(true);
            expect(updatedDoc?.pages.find((p) => p.id === pageId)?.rotation).toBe(45);
        });

        it('should prevent viewerRotation when hasMachineTextLayer is true', () => {
            const documents = mockDocumentEntities();
            const doc = documents[0];
            const pageId = doc.pages[0].id;
            doc.pages[0].hasMachineTextLayer = true;
            doc.pages[0].viewerRotation = 0;
            ...
            const action = userActions.updatePagesRotation({
                pages: [{ pageId, documentId: doc.id, viewerRotation: 90 }],
                taskDataSynced: false,
            });

            const state = documentReducer(mockDocumentState, action);
            const updatedDoc = state.entities[doc.id];
            const updatedPage = updatedDoc?.pages.find((p) => p.id === pageId);

            // ViewerRotation should remain unchanged (not updated) when hasMachineTextLayer is true
            expect(updatedPage?.viewerRotation).toBe(0);
            expect(updatedPage?.hasMachineTextLayer).toBe(true);
        });

        it('should allow viewerRotation when hasMachineTextLayer is false', () => {
            const documents = mockDocumentEntities();
            const doc = documents[0];
            const pageId = doc.pages[0].id;
            doc.pages[0].hasMachineTextLayer = false;
            doc.pages[0].viewerRotation = 0;
            ...
            // ViewerRotation should be applied when hasMachineTextLayer is false
            expect(updatedPage?.viewerRotation).toBe(90);
            expect(updatedPage?.hasMachineTextLayer).toBe(false);
        });

        it('should prevent viewerRotation when hasMachineTextLayer is set to true in the same update', () => {
            const documents = mockDocumentEntities();
            const doc = documents[0];
            const pageId = doc.pages[0].id;
            doc.pages[0].viewerRotation = 45;
            ...
            const action = userActions.updatePagesRotation({
                pages: [{ pageId, documentId: doc.id, hasMachineTextLayer: true, viewerRotation: 90 }],
                taskDataSynced: false,
            });
    ...
    it('should store cut page ids and reset last action on page cut', () => {
        const documents = mockDocumentEntities();
        ...
        const cutPageIds = [documents[0].pages[0].id, documents[1].pages[0].id];
        const action = userActions.pageCut({
            pageIds: cutPageIds,
        });

        const state = documentReducer(mockDocumentState, action);

        expect(state.cutPageIds).toEqual(cutPageIds);
        expect(state.lastAction).toBeUndefined();
    });

    it('should select cut pages and clear them on insert page success', () => {
        const mockDocumentState: DocumentState = {
            ...initialDocumentState,
            cutPageIds: ['page-1', 'page-2'],
            selectedPageIds: ['existing-selection'],
            lastAction: {} as any,
        };

        const action = systemActions.documentOperationSuccess({
            docAction: IdpDocumentAction.InsertPage,
            canUndoAction: true,
            updates: [],
            contextPageIds: [],
            notificationMessage: '',
        ...
        expect(state.selectedPageIds).toEqual(['page-1', 'page-2']);
        expect(state.cutPageIds).toEqual([]);
        expect(state.lastAction).toBeUndefined();
    });

    it('should keep cut pages untouched for non insert document operation success', () => {
        const mockDocumentState: DocumentState = {
            ...initialDocumentState,
            cutPageIds: ['page-1'],
            selectedPageIds: ['existing-selection'],
        ...
        expect(state.cutPageIds).toEqual(['page-1']);
        expect(state.selectedPageIds).toEqual(['existing-selection']);
        expect(state.lastAction?.action).toEqual(IdpDocumentAction.Merge);
        expect(state.lastAction?.isSuccess).toBeTrue();
    });
});

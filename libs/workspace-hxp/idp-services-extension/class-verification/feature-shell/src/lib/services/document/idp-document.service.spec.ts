...
describe('IdpDocumentService', () => {
    ...
    it('should dispatch on remove document flag', () => {
        const documentAction = IdpDocumentAction.Reject;
        const canUndoAction = true;
        const documents = mockIdpDocuments();
        const pages = [documents[0].pages[0], documents[1].pages[0]];

        service.removeDocumentFlag(documentAction, canUndoAction, pages);

        const action = userActions.documentReject({ docAction: documentAction, canUndoAction, pages });
        expect(storeDispatchSpy).toHaveBeenCalledWith(action);
    });
    ...
    it('should dispatch on create page copies', () => {
        const documentAction = IdpDocumentAction.CreateCopy;
        const canUndoAction = true;
        const documents = mockIdpDocuments();
        const pages = [documents[0].pages[0], documents[1].pages[0]];

        service.createPageCopies(documentAction, canUndoAction, pages);

        const action = userActions.pageCreateCopy({ docAction: documentAction, canUndoAction, pages });
        expect(storeDispatchSpy).toHaveBeenCalledWith(action);
    });

    it('should dispatch pageCut with selected page ids when cutting selected pages', fakeAsync(() => {
        const selectedPages = [mockDocuments[0].pages[0], mockDocuments[1].pages[1]];
        store.overrideSelector(selectAllSelectedPages, selectedPages);
        store.refreshState();

        service.cutSelectedPages();
        flush();

        expect(storeDispatchSpy).toHaveBeenCalledWith(userActions.pageCut({ pageIds: selectedPages.map((page) => page.id) }));
    }));

    it('should dispatch pageCut with empty list when clearing cut pages', () => {
        service.clearCutPages();

        expect(storeDispatchSpy).toHaveBeenCalledWith(userActions.pageCut({ pageIds: [] }));
    });

    it('should move cut pages into target document on insert', fakeAsync(() => {
        const cutPages = [mockDocuments[2].pages[0], mockDocuments[2].pages[1]];
        store.overrideSelector(selectAllCutPages, cutPages);
        store.refreshState();

        service.insertCutPages('target-doc', 3);
        flush();

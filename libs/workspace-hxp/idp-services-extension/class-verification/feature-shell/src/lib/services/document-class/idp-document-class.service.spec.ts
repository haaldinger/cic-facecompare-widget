...
describe('IdpDocumentClassService', () => {
    ...
    it('should filter out classes marked as ignoreForReview', fakeAsync(() => {
        const classes = mockIdpConfigClasses();
        const mockClassesMetadata: DocumentClassMetadata[] = [
            { ...classes[0], documentsCount: 2, issuesCount: 0, canExpand: true, disabled: false, ignoreForReview: true },
            { ...classes[1], documentsCount: 1, issuesCount: 0, canExpand: true, disabled: false },
            { ...classes[2], documentsCount: 3, issuesCount: 1, canExpand: true, disabled: false },
        ];
        store.overrideSelector(selectClassMetadata, mockClassesMetadata);
        store.overrideSelector(selectViewFilter, 'All');
        store.refreshState();
        ...
        expect(result.some((c) => c.id === classes[0].id)).toBeFalse();
        expect(result.map((c) => c.id)).toEqual(jasmine.arrayContaining([classes[1].id, classes[2].id]));

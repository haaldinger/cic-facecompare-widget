...
describe('DocumentLocationSearchFilterService', () => {
    ...
    it('should return correct HXQL for a single path', () => {
        const data = {
            values: [{ label: '', value: jestMocks.folderDocument.sys_path }],
        } as DocumentLocationSearchFilterData;

        const expectedHXQL = `(sys_path STARTSWITH '${jestMocks.folderDocument.sys_path}')`;
    ...
    it('should return correct HXQL for multiple paths', () => {
        const data = {
            values: [
                { label: '', value: jestMocks.nestedDocumentAncestors[0].sys_path },
                { label: '', value: jestMocks.nestedDocumentAncestors[1].sys_path },
            ],
        } as DocumentLocationSearchFilterData;
    ...
    it('should search for documents when search term is provided', async () => {
        jest.spyOn(mockSearchService, 'sanitizeQuery').mockReturnValue('folder');
        jest.spyOn(mockSearchService, 'getDocumentsByQuery').mockReturnValue(of(jestMocks.searchResults));

        expect(mockSearchService.getDocumentsByQuery).not.toHaveBeenCalled();

        const searchTerm = 'folder';
        const searchResultDocuments = await firstValueFrom(searchInFilterService.searchDocuments(searchTerm));

        expect(searchResultDocuments).toBeTruthy();
        expect(searchResultDocuments).toHaveLength(jestMocks.searchResults.documents.length);
        expect(searchResultDocuments).toEqual(jestMocks.searchResults.documents);

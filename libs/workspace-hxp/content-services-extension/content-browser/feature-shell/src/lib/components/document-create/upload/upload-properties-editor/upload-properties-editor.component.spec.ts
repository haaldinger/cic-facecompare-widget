...
describe('ContentUploadPropertiesEditorComponent', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(mockModelApi, 'getModel')
    ...
    it('save button should be enabled if some properties were edited', () => {
        ...
        locationElement.componentInstance.document = jestMocks.folderDocument;
        locationElement.componentInstance.selectedLocation.emit(jestMocks.folderDocument);
    ...
    it('should save properties on edit', async () => {
        ...
        const documentsUpdatedSpy = jest.spyOn(component.documentsUpdated, 'emit');
        const savePropertiesButton = fixture.debugElement.query(By.css('#hxp-upload-properties-editor-save'));
    ...
    it('should emit updated data with new properties when saving', async () => {
        component.data = generateMockUploadData();
        component.currentDocument = jestMocks.folderDocument;
        component.ngOnChanges();
        fixture.detectChanges();
        ...
        const emittedData = documentsUpdatedSpy.calls.mostRecent().args[0];

        expect(emittedData).toBeDefined();
        expect(emittedData.length).toBe(component.data.length);
        expect(emittedData[0].documentModel.document.sys_primaryType).toBe('SysFile');
        expect(emittedData[0].documentModel.document.sys_parentPath).toBe(jestMocks.folderDocument.sys_path);
    });

...
describe('UploadSnackbarComponent', () => {
    ...
    let queueEmptied: Subject<void>;

    beforeEach(async () => {
        (mockUploadManagerService as any).queueChanged = new Subject();
        (mockUploadManagerService as any).uploadError = new Subject();
        (mockUploadManagerService as any).uploadCanceled = new Subject();
        (mockUploadManagerService as any).uploadCompleted = new Subject();
        (mockUploadManagerService as any).uploadRetried = new Subject();
        queueEmptied = new Subject();
        (mockUploadManagerService as any).queueEmptied = queueEmptied;
        (mockUploadManagerService as any).queueEmptied$ = queueEmptied.asObservable();
        mockUploadSnackbarService.maximize$ = new Subject();
        mockUploadSnackbarService.minimize$ = new Subject();

        jest.spyOn(mockUploadSnackbarService, 'requestMaximize').mockImplementation(() => (mockUploadSnackbarService.maximize$ as Subject<void>).next());
        jest.spyOn(mockUploadSnackbarService, 'requestMinimize').mockImplementation(() => (mockUploadSnackbarService.minimize$ as Subject<void>).next());
    ...
    it('should display snackbar on queue changed and maximize', () => {
        jest.spyOn(mockUploadManagerService, 'isUploadOngoing').mockReturnValue(true);
    ...
    it('should cancel all ongoing uploads', () => {
        jest.spyOn(mockUploadManagerService, 'isUploadOngoing').mockReturnValue(true);
        jest.spyOn(mockUploadManagerService, 'isUploadCompleted').mockReturnValue(false);

        const cancelAllUploadsSpy = jest.spyOn(fixture.componentInstance, 'cancelAllUploads');
    ...
    it('should close the snackbar when all uploads have finished', () => {
        jest.spyOn(mockUploadManagerService, 'isUploadOngoing').mockReturnValue(false);
        jest.spyOn(mockUploadManagerService, 'isUploadCompleted').mockReturnValue(true);

        const closeSnackbarSpy = jest.spyOn(fixture.componentInstance, 'close');
    ...
    it('should close snackbar when queueEmptied is emitted', () => {
        const componentInstance = fixture.componentInstance;
        const data = generateMockUploadData(2);
        mockUploadManagerService.queueChanged.next(data);
        fixture.detectChanges();
        mockUploadSnackbarService.requestMaximize();
        fixture.detectChanges();
        ...
        expect(snackbar).toBeTruthy();
        expect(componentInstance.isDialogActive).toBe(true);
        expect(componentInstance.uploadDataList).toHaveLength(2);

        queueEmptied.next();
        fixture.detectChanges();

        expect(componentInstance.isDialogActive).toBe(false);
        expect(componentInstance.uploadDataList).toHaveLength(0);

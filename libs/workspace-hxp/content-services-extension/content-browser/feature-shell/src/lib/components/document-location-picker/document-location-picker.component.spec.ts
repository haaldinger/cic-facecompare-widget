...
import { DocumentLocationPickerHarness } from './document-location-picker-harness.mock';
...
describe('DocumentLocationPickerComponent', () => {
    let componentRef: ComponentRef<DocumentLocationPickerComponent>;
                ...
                MockProvider(DocumentService, {
                    getFolderChildren: () =>
                        of({
                            ...
                            limit: 10,
                            offset: 0,
                            totalCount: jestMocks.nestedDocumentAncestors.length,
                        }),
    ...
    it('should display document location if one is provided as input', async () => {
        ...
        componentRef.setInput('document', jestMocks.folderDocument);
        fixture.detectChanges();

        input = await documentLocationHarness.getInput();

        expect(await input?.getPlaceholder()).toBe(jestMocks.folderDocument.sys_path);
    });

    it('should select a location when a new document is selected', async () => {
        const documentLocationHarness = await loader.getHarness(DocumentLocationPickerHarness);
        const baseLocation = jestMocks.folderDocument;
        const selectedLocation = jestMocks.nestedDocumentAncestors[1];

        componentRef.setInput('document', baseLocation);
        fixture.detectChanges();

...
import { HxpFileUploadErrorPipe, HxpUploadService, FileReaderService } from '@hxp/workspace-hxp/shared/upload-files/feature-shell';
import { MockPipe, MockProvider } from 'ng-mocks';
import { DocumentService, DocumentModelService } from '@alfresco/adf-hx-content-services/services';
...
describe('UploadSnackbarListRowComponent', () => {
    ...
    beforeEach(async () => {
        const documentModelServiceSpy = { getModel: jest.fn() };
        const mockModel = {
            hasMixin: jest.fn().mockReturnValue(false)
        };
        const modelSubject = new BehaviorSubject(mockModel);
        ...
        await TestBed.configureTestingModule({
            ...
            imports: [CommonModule, MatProgressBarModule, MatIconTestingModule, NoopTranslateModule, MatTooltipModule],
            providers: [
    ...
    it('should cancel an ongoing upload', () => {
        const upload = generateMockUploadData(1)[0];
        const cancelUploadSpy = jest.spyOn(uploadManagerService, 'cancelUpload');
    ...
    it('should trigger upload retry', () => {
        const upload = generateMockUploadData(1)[0];
        const retryUploadSpy = jest.spyOn(uploadManagerService, 'retryUpload');

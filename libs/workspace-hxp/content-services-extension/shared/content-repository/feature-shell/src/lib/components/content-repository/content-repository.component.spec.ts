...
import { a11yReport, jestMocks as mocks } from '@hxp/workspace-hxp/shared/testing';
import { NoopAnimationsModule } from '@angular/platform-browser/animations';
import { UPLOAD_MIDDLEWARE_SERVICE } from '@hxp/shared-hxp/services';
import { Document } from '@hylandsoftware/hxcs-js-client';
import { DebugElement, LOCALE_ID, QueryList, signal } from '@angular/core';
...
import { CONTEXT_MENU_ACTIONS_PROVIDERS, TableSkeletonLoaderComponent } from '@alfresco/adf-hx-content-services/ui';
...
const EXPECTED_VIOLATIONS: { [violationId: string]: number }[] | undefined = [{ 'aria-required-children': 3 }, { 'aria-valid-attr-value': 1 }];
...
describe('ContentRepositoryComponent', () => {
    ...
    beforeEach(() => {
        ...
        TestBed.overrideComponent(ContentRepositoryComponent, {
            remove: { imports: [DocumentActionToolbarComponent, ContentUploadDragAreaComponent] },
            add: { imports: [MockComponent(DocumentActionToolbarComponent), MockComponent(ContentUploadDragAreaComponent)] },
        });

        jest.spyOn(mockDocumentService, 'getDocumentById').mockImplementation((id: string) => {
            if (id === ROOT_DOCUMENT.sys_id) {
                return of(ROOT_DOCUMENT);
            }
            if (id === mocks.folderDocument.sys_id) {
                return of(mocks.folderDocument);
            }
            return of(undefined);
        });
    ...
    it('should clears selections when request', () => {
        jest.spyOn((component as any).documentListElement, 'resetSelection');
    ...
    it('should clear selection on page navigate', () => {
        jest.spyOn(component as any, 'resetTableSelection');
    ...
    it('should update updatedDocument on document update', fakeAsync(() => {
        ...
        jest.spyOn(mockDocumentService, 'getAncestors').mockReturnValue(of([]));
        jest.spyOn(mockDocumentService, 'updateDocument');
    ...
    it('should reselect a document when it has been updated', fakeAsync(() => {
        const selectRowOfDocumentSpy = jest.spyOn(component as any, 'selectRowOfDocument');
        component.documents = mocks.nestedDocumentAncestors;
        fixture.detectChanges();
        ...
        expect(selectRowOfDocumentSpy).toHaveBeenCalledTimes(1);
        expect(selectRowOfDocumentSpy).toHaveBeenCalledWith(updatedDocument);
    ...
    it('should update selection and actionContext when a selected document is updated', async () => {
        const originalDocument = mocks.nestedDocumentAncestors[0];
        const updatedDocument = { ...originalDocument, sys_mixinTypes: ['SysVersionable'] } as Document;
        const updatedProperties = new Map<string, any>([['sys_primaryType', 'someValue']]);

        component.documents = mocks.nestedDocumentAncestors;
        component.actionContext = { documents: [originalDocument] };
        (component as any).selection = [originalDocument];
        fixture.detectChanges();

        (mockDocumentService.documentUpdated$ as Subject<DocumentUpdateInfo>).next({ document: updatedDocument, updatedProperties });
        await fixture.whenStable();

        expect((component as any).selection).toEqual([updatedDocument]);
        expect(component.actionContext.documents).toEqual([updatedDocument]);
    });

    it('should not update selection when an unselected document is updated', async () => {
        const selectedDocument = mocks.nestedDocumentAncestors[0];
        const otherDocument = { ...mocks.nestedDocumentAncestors[1], sys_mixinTypes: ['SysVersionable'] } as Document;
        const updatedProperties = new Map<string, any>([['sys_primaryType', 'someValue']]);

        component.documents = mocks.nestedDocumentAncestors;
        component.actionContext = { documents: [selectedDocument] };
        (component as any).selection = [selectedDocument];
        fixture.detectChanges();

        (mockDocumentService.documentUpdated$ as Subject<DocumentUpdateInfo>).next({ document: otherDocument, updatedProperties });
        await fixture.whenStable();

        expect((component as any).selection).toEqual([selectedDocument]);
    });

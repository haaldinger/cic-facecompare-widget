...
import { PendingDocument } from '@hxp/shared-hxp/services';
...
describe('FormWidgetService', () => {
    ...
    describe('getDocumentFromField', () => {
        it('should return null when field value is null', async () => {
            const document = await firstValueFrom(service.getDocumentFromField(field));
    ...
    describe('getDocumentsFromField', () => {
        it('should return empty array when field value is null', async () => {
            const document = await firstValueFrom(service.getDocumentsFromField(field));
            expect(document).toEqual([]);
        });

        it('should return PendingDocument directly without calling document service', async () => {
            ...
            const documentByIdSpy = spyOn(TestBed.inject(DOCUMENT_SERVICE), 'getDocumentById').and.callThrough();
            field.value = [pendingDocument];

            const result = await firstValueFrom(service.getDocumentsFromField(field));

            expect(result).toEqual([pendingDocument]);
            expect(documentByIdSpy).not.toHaveBeenCalled();
        });

        it('should handle mixed array of PendingDocument and plain document references', async () => {
            const pendingDocument: PendingDocument = {
                document: { ...DOCUMENT_MOCK },
                originalPermissions: [],
                pendingBy: 'user-123', persisted: false,
            ...
            field.value = [pendingDocument, { sys_id: DOCUMENT_MOCK.sys_id }];

            const result = await firstValueFrom(service.getDocumentsFromField(field));

            expect(result.length).toBe(2);
            expect(result[0]).toBe(pendingDocument);
        });
    ...
    describe('getViewerContentFromDocument', () => {
        it('should return null when document is null', async () => {
            const viewerContent = await firstValueFrom(service.getViewerContentFromDocument(null));
            expect(viewerContent).toBeNull();
        });

        it('should return null when document has no sys_id null', async () => {
            const viewerContent = await firstValueFrom(service.getViewerContentFromDocument({ ...DOCUMENT_MOCK, sys_id: undefined }));
            expect(viewerContent).toBeNull();
        });
        ...
        it('should download current document blob when document mimetype is supported', async () => {
            ...
            const viewerContent = await firstValueFrom(service.getViewerContentFromDocument(DOCUMENT_MOCK));
        ...
        it('should request the rendition creation when document mimetype is not supported and rendition does not exist', async () => {
            ...
            const viewerContent = await firstValueFrom(service.getViewerContentFromDocument(document));

...
import { UserLayoutOptions, userLayoutOptionsFromString } from '../models/layout';
...
export class ViewerToolbarService {
    ...
    private readonly focusToolbarItemSubject$ = new Subject<ToolbarItemTypes>();
    readonly focusToolbarItem$ = this.focusToolbarItemSubject$.asObservable();
        ...
        this.viewerToolbarItems$ = combineLatest([
            ...
            this.viewerService.viewerState$.pipe(
                map((state) => ({
                    ...
                    redactionDrawMode: state.redactionDrawMode,
                })),
                distinctUntilChanged(
                    (prev, curr) =>
                        ...
                        prev.currentPageId === curr.currentPageId &&
                        prev.showRedaction === curr.showRedaction &&
                        prev.redactionDrawMode === curr.redactionDrawMode
                )
            ),
            this.viewerService.datasource$,
        ]).pipe(
            map(([layout, viewerState, datasource]) => {
                const allPages = datasource.documents.flatMap((doc) => doc.pages);
                const isSinglePageMode = viewerState.currentLayout.type === UserLayoutOptions.SinglePage;
                const currentPageIndex = viewerState.pageNavInfo.currentPageIndex;
                const currentPage = currentPageIndex === undefined ? undefined : allPages[currentPageIndex];
                const hasPagesWithRotationDisabled = isSinglePageMode
                    ? currentPage?.disableRotation === true
                    : allPages.length > 0 && !allPages.some((page) => !page.disableRotation);
                ...
                return Object.values(ToolbarConfig).map((item) => {
                    ...
                    const shortcutActionMap: Record<ToolbarItemTypes, ViewerShortcutAction | undefined> = {
                        ...
                        [ToolbarItemTypes.RedactionToggle]: ViewerShortcutAction.RedactionToggle,
                        [ToolbarItemTypes.RedactionDrawMode]: ViewerShortcutAction.RedactionDrawMode,
                    };
                    ...
                    if (
                        item.type === ToolbarItemTypes.Rotate &&
                        (viewerState.currentLayer === ViewerLayerType.TextOnly || hasPagesWithRotationDisabled)
                    ) {
                        item.enabled = false;
                    }

                    if (item.type === ToolbarItemTypes.RedactionToggle) {
                        item = {
                            ...item,
                            icon: viewerState.showRedaction ? 'view_off' : 'view',
                            selected: viewerState.showRedaction,
                        };
                    }

                    if (item.type === ToolbarItemTypes.RedactionDrawMode) {
                        item = {
                            ...item,
                            selected: viewerState.redactionDrawMode,
                        };
                    }
                    ...
                    return {
                        ...item,
                        selected:
                            item.type === ToolbarItemTypes.RedactionToggle || item.type === ToolbarItemTypes.RedactionDrawMode
                                ? item.selected
                                : isSelected,
    ...
    private calculateNewActionValue(
        ...
        switch (action) {
            ...
            case ViewerShortcutAction.RedactionToggle: {
                return { showRedaction: !currentState.showRedaction };
            }
            case ViewerShortcutAction.RedactionDrawMode: {
                return { redactionDrawMode: !currentState.redactionDrawMode };
            }
            default: {
                return undefined;
            }
    ...
    requestFocusOnToolbarItem(itemType: ToolbarItemTypes) {
        this.focusToolbarItemSubject$.next(itemType);
    }
}

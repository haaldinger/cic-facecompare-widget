...
describe('IdpThumbnailViewerComponent', () => {
    ...
    const createViewerShortcutServiceMock = (action: ViewerShortcutAction) => ({
        getShortcutForEvent: jest.fn().mockReturnValue({
            action,
            key: '',
            modifierKeys: [],
    ...
    const setupTestModule = (action: ViewerShortcutAction, currentPageIndex = 0) => {
        const viewerServiceMock = {
            ...
            datasource$: new BehaviorSubject({
                ...
                loadThumbnailFn: jest.fn(),
            }),
            viewerKeyboardEvent$: mockKeyboard$.asObservable(),
            changePageSelection: jest.fn(),
            changeToolbarItemSelectionState: jest.fn(),
        };

        const viewerToolbarServiceMock = {
            requestFocusOnToolbarItem: jest.fn(),
        };

        TestBed.configureTestingModule({
            imports: [IdpThumbnailViewerComponent, NoopTranslateModule, MatIconTestingModule],
            providers: [
                { provide: ViewerService, useValue: viewerServiceMock },
                { provide: ViewerToolbarService, useValue: viewerToolbarServiceMock },
    ...
    it('should call closePanel with restoreFocus=true when Escape is pressed', fakeAsync(() => {
        setupTestModule(ViewerShortcutAction.NavigateDown);
        ...
        expect(closePanelSpy).toHaveBeenCalledWith(true);
    }));
    ...
    it('should scroll to selected page when page index changes while panel is expanded', fakeAsync(() => {
        setupTestModule(ViewerShortcutAction.NavigateDown, 0);

        const scrollToSelectedPageSpy = jest.spyOn(component, 'scrollToSelectedPage');
        component.pageItems$.next(generatePageItems(0));
        fixture.detectChanges();

        (viewerService.viewerState$ as BehaviorSubject<any>).next({
            pageNavInfo: { currentPageIndex: 2 },
            selectedToolbarItems: [ToolbarConfig.ThumbnailViewer.type],
        });
        ...
        expect(scrollToSelectedPageSpy).toHaveBeenCalled();
    }));
});

...
export class ProcessFilterService {
    ...
    getFilters(processFilterCloud: ProcessFilterCloudModel): Observable<Filter[]> {
        ...
        return combineLatest([
            this.fetchProcessDefinitionNames(),
    ...
    saveFilterAs(processFilterCloud: ProcessFilterCloudModel, newFilterName: string): Observable<ProcessFilterCloudModel[]> {
        ...
        const filterKey = newFilterName.trim().replaceAll(new RegExp(' ', 'g'), '-').toLowerCase();
        ...
        for (const f of filters) {
            ...
            switch (f.name) {
                ...
                case ProcessFilterKey.APP_VERSIONS: {
                    ...
                    filterValues['appVersion'] = value?.length ? Number.parseInt(value[0] || '', 10) : null;
                    break;
                }
    ...
    private fetchProcessDefinitionNames(): Observable<string[]> {
        return this.store
            .select(selectProcessDefinitionEntities)

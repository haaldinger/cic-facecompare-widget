...
export class ProcessListByCategoryComponent implements OnInit, OnChanges {
    ...
    private readonly categoryMapper = inject<SortByCategoryMapperService<ListByCategoryProcess>>(SortByCategoryMapperService);
    ...
    private filterProcessesBySearchText(processes: ListByCategoryProcess[], searchValue: string): ListByCategoryProcess[] {
        return processes.filter((process) => process.name.toLocaleLowerCase().includes(searchValue.toLocaleLowerCase()));
    }

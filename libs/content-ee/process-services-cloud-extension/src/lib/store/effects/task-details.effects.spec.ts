...
describe('TaskDetailsEffects', () => {
    ...
    it('Should show error notification on taskAssignmentFailure action', async () => {
        ...
        await firstValueFrom(effects.taskAssignmentFailure$);
    ...
    it('should call redirectForStartProcess when startFormRedirection action', () => {
        actions$ = hot('-a-', {
            a: startFormCompletedRedirection({
                appName: 'mock-appName',
                processDefinitionName: 'mock-process',
                redirectParameter: 'mock-parameter',
                selectedOutcomeId: 'custom-outcome',
            }),
        });
        ...
        effects.startFormRedirection$.subscribe(() => {
            expect(taskRedirectionSpy).toHaveBeenCalledWith('mock-appName', 'mock-process', 'mock-parameter', 'custom-outcome');
        });
    });

    it('should call redirectForTask when startFormRedirection action', () => {
        actions$ = hot('-a-', { a: taskCompletedRedirection({ taskId: 'mock-taskId', selectedOutcomeId: 'custom-outcome' }) });
        ...
        effects.taskCompletedRedirection$.subscribe(() => {
            expect(taskRedirectionSpy).toHaveBeenCalledWith('mock-taskId', 'custom-outcome');
        });
    });
});
